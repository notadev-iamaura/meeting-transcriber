...
_BRAND_PATTERNS: set[str] = {
    ...
    "삼성", "삼양", "삼화", "삼천리", "삼립", "삼국",
    # 일(1)로 시작
    "일동", "일진", "일양", "일단", "일반", "일부", "일종", "일정",
    "일시", "일괄", "일체", "일치", "일률", "일상", "일방", "일련",
    "일요일", "일본",
    # 이(2)로 시작
    "이마트", "이화", "이랜드", "이번", "이후", "이전", "이상", "이하",
    "이미", "이제", "이런", "이것", "이때", "이날", "이를", "이유",
    "이해", "이익", "이용", "이동", "이어", "이내", "이래",
    # 사(4)로 시작
    "사실", "사업", "사용", "사람", "사이", "사항", "사무", "사과",
    "사장", "사회", "사건", "사례", "사전", "사후", "사태", "사망",
    "사고", "사기", "사정", "사진",
    # 오(5)로 시작
    "오히려", "오늘", "오전", "오후", "오래", "오뚜기", "오리온",
    "오류", "오해", "오직",
    # 육(6)으로 시작
    "육성", "육아", "육지",
    # 칠(7)로 시작
...
# 예: "삼십", "이백오십", "이천이십육", "오백만"
_KR_NUMBER_PATTERN = re.compile(
    rf"([{_KR_NUM_CHARS}]{{1,12}})"
)

# 혼합형 패턴: 아라비아 숫자 + 한글 자리수 (예: "3십", "2백", "15만")
_MIXED_PATTERN = re.compile(
    r"(\d{1,15})(십|백|천|만|억|조)"
)

# 고유명사 보호 패턴: 가장 긴 것부터 매칭 (탐욕적)
_BRAND_REGEX = re.compile(
    "|".join(
        re.escape(brand) for brand in
        sorted(_BRAND_PATTERNS, key=len, reverse=True)
    )
)
...
def _parse_korean_number(korean: str) -> int:
    ...
    # 큰 단위(조 → 억 → 만) 순서로 처리
    for unit_name, unit_value in sorted(
        _LARGE_UNITS.items(), key=lambda x: x[1], reverse=True
    ):
