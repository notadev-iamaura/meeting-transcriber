...
#
# POST /api/meetings/batch — 전사·요약·full 의 일괄 처리를 통합한 단일 엔드포인트.
# 보안 및 성능 패치:
#   * Phase 6 Medium-01: meeting_ids 길이 상한 (max_length=500) 으로 DoS 차단
#   * Phase 6 Medium-02: audio_path 가 base_dir 외부를 가리키면 사전 제외
#   * Phase 6 Perf C-1: 체크포인트 디렉토리 스캔과 분류를 asyncio.to_thread 로
#                       이동해 이벤트 루프 블로킹 제거
#   * Phase 6 Perf M-1: base_dir.resolve() 를 호출자가 1회 캐시하여 회의별 반복
#                       resolve() 비용 제거
#   * Phase 3 Major #1: meeting_ids 중복 제거 (LLM 토큰 낭비 방지)
#   * Phase 3 Major #2: transcribe 분기에서 audio_path 부재 회의를 사전 제외해
#                       응답의 queued 카운트가 실제 실행 수와 일치하도록 보정


from typing import Literal as _BatchLiteral  # noqa: E402  (이 섹션 내부 사용)
...
class BatchActionRequest(BaseModel):
    ...
    action: _BatchLiteral["transcribe", "summarize", "full"]
    scope: _BatchLiteral["all", "recent", "selected"]
...
class BatchActionResponse(BaseModel):
    ...
    status: _BatchLiteral["ok", "no_targets"]
    message: str
    action: str
    scope: str
    matched: int
...
def _classify_meeting_for_batch(
    ...
    outputs_dir: Path,
    meeting_id: str,
) -> _BatchLiteral["transcribe", "summarize", "done"]:
    """회의의 현재 진행 단계를 분류한다.
