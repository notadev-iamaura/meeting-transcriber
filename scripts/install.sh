...
print_summary() {
    ...
    echo ""
    echo "  2. HuggingFace CLI 로그인 (화자분리에 필요, LaunchAgent 호환):"
    echo "     hf auth login"
    echo "     (https://huggingface.co/settings/tokens 에서 토큰 발급 후 CLI에만 입력)"
    echo "     토큰은 ~/.cache/huggingface/token의 소유자 전용 캐시에만 저장하세요."
    echo ""
