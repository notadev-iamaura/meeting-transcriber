    ...
    index = _index(
        tmp_path,
        {
            ...
            "decisions/superseded.md": _decision_md(
                ...
                confidence=10,
                body=("예산 예산 재조정. [meeting:1234abcd@00:01:20] [meeting:1234abcd@00:02:30]"),
            ),
...
def test_enabled_플래그가_BM25순서와_재랭킹순서를_정반대로_바꾼다(tmp_path: Path) -> None:
    ...
    order_off = [
        r.page_path for r in _index(tmp_path / "off", pages, off).search("예산", now=_NOW)
    ]
    ...
    index = _index(
        tmp_path,
        {
            "decisions/best.md": _decision_md(
                ...
                decision_date="2026-05-30",
                confidence=10,
                body=("예산 확정. [meeting:1234abcd@00:01:20] [meeting:1234abcd@00:02:30]"),
            ),
