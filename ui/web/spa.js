...
(function () {
    ...
    WikiView.prototype._toggleCategory = function (catId) {
        ...
        var node = this._els.tree.querySelector(
            '.wiki-tree__category[data-cat="' + CSS.escape(catId) + '"]'
        );
        if (node) {
    ...
    WikiView.prototype._closeHealthModal = function () {
        ...
        // 배지 버튼으로 focus 복귀
        if (this._els && this._els.healthBadge) {
    ...
    function isEditingContext(target) {
        ...
        // T-202: native <dialog class="command-palette"> 컨테이너 안의 input 은
        // 팔레트 자체 핸들러가 처리하므로 전역 단축키 가로채기 대상에서 제외.
        var inPalette =
