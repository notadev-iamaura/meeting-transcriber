...
- 중복 일괄 작업 버튼을 목록 위 선택 도구 모음 하나로 정리했습니다. 현재 열린
  녹취와 체크한 녹취의 표시를 구분하고, 상세 화면의 보조 작업은 `더 보기`로 모았습니다.
- `제목 자동 정리`는 개별·일괄 모두 모달 없이 접수하고 날짜를 포함한 제목을
  백그라운드에서 자동 적용합니다. 화면 이동·새로고침 후에도 진행을 확인할 수 있습니다.
- 작업 내역에서 중단·실패 재시도·이전 제목 복원을 지원합니다. 접수 뒤 수동 편집한
  제목이나 바뀐 전사문은 덮어쓰지 않으며, 앱 종료 후 미완료 작업은 중단으로 표시합니다.
- 실제 Gemma E4B로 격리된 짧은 전사문 두 건의 API 접수·생성·저장·복원을 확인했습니다.
  총 21.05초였으며 원본·전사문 hash를 보존했고 모델·native 정리 대기가 남지 않았습니다.
  실제 사용자 녹취의 대규모 처리나 제목 품질 평가는 포함하지 않습니다.
- UI/API/SQLite 통합 E2E 5개(모델 응답 대역), 제목·큐·변경 잠금·모델 수명 관련
  187개, 추가 종료 경합·전역 모델 정리 테스트 포함 제목 테스트 14개를 통과했습니다.
  릴리스 하네스는 152 passed / 1 skipped입니다. 전체 Ruff·format과 production
  mypy(Python 3.12 대상)를 통과했습니다.
- 기본 전체 테스트는 `pytest tests/ -q`로 4,069 passed / 1 skipped입니다.
  UI·E2E·native 등 별도 실행 대상 254개는 이 명령에서 제외됩니다.
- 일괄 UI 동작·접근성·화면 회귀 52개를 통과했습니다. 제목 입력 중 AI 완료가
  도착해도 입력·초점을 유지하며, 내역 갱신도 키보드 초점·스크롤을 보존합니다.
- 동작·복구·검증 범위: [백그라운드 제목 정리](design-decisions/background-title-work.md).
...
### 일괄 처리 확인·접수 내역과 단계별 실패 표시 (2026-09-08, 로컬 변경)

- 선택한 회의 목록 바로 위에 일괄 작업을 표시하고, 후보별 선택·해제와
  선택/실행 가능/제외 건수를 확인한 뒤 큐에 등록합니다. 최근 24시간은
  앱 등록 시각 기준입니다. 접수 내역은 SQLite에 남아 다시 조회할 수 있습니다.
- 전사 완료 후 AI 교정 실패를 별도로 표시하고 전사문 열기·교정 재시도를
  제공합니다. 별도로 실행한 LLM 실패도 같은 상태로 표시하며 STT 재호출을 피합니다.
- Gemma projection 가중치 불일치에 업데이트·모델 변경 안내를 추가하고,
  기존 `mlx-vlm` 의존성의 하한을 작은 실제 MLX 로딩 검증을 통과한 0.6.17로 올렸습니다.
- 외부 사례의 실행 이력은 우리 서비스의 실제 장애로 확인된 사실이 아닙니다.
  검증은 격리 환경을 사용하며 모델 추론 품질과 전체 E4B 실행을 포함하지 않습니다.
- 구현 계약, 순서 비교, 재현 명령과 한계는
  [일괄 처리 선택·접수·단계 표시](design-decisions/batch-progress.md)에 기록했습니다.
- 검증: 기본 테스트 3,930 passed / 1 skipped, harness 152 passed,
  브라우저 E2E 17 passed, 일괄 UI 동작 30 passed, 접근성·시각 회귀 18 passed.
  실제 MLX의 작은 Gemma projection 검증 1 passed, 전체 Ruff·포맷 검사,
  Python 3.12 대상 mypy와 변경 JS 문법 검사를 통과했습니다. 시각 기준 이미지
  5개는 변경된 버튼·선택 도구·문구를 직접 확인한 뒤 갱신했습니다.

### OpenAI 전사 실패 안내 보강 (2026-09-07)

- 실패한 회의는 부분 전사문 유무와 관계없이 상단에 오류와 복구 안내를 표시합니다.
  OpenAI 실패 시 ‘이 Mac에서 로컬로 다시 전사할까요?’와 실행 버튼을 제공합니다.
  산출물 교체를 확인한 사용자만 `re-transcribe`에 `model_id=local`을 보내며,
  회의별 queue snapshot을 로컬로 고정하고 전역 기본값은 보존합니다. 취소 시 요청하지 않습니다.
- HTTP 400/401/403/404/413/429/5xx별 안내를 구분하고, 429의 명시적인
  `insufficient_quota`는 결제·사용 한도 확인으로 안내합니다. 구체적인
  upstream 원인은 추정하지 않으며 응답 본문과 API 키는 오류 메시지에 노출하지 않습니다.
- 실제 400 실패의 원인 분석은 미완료입니다. 격리 SQLite에 저장한 실패 상태로
  브라우저 오류 표시·취소 시 POST 없음·동의 후 실제 로컬 큐잉과 설정 보존을 검증합니다.
- 로컬 검증: 기본 테스트 3,922 passed / 1 skipped, harness 152 passed / 1 skipped,
  편집·실패 복구 Playwright E2E 15 passed. 전체 Ruff 검사·포맷 검사,
  Python 3.12 대상 mypy, 뷰어 JS 문법 검사를 통과했습니다.
  실제 OpenAI 요청과 로컬 모델의 음성 전사 완료는 이번 검증에 포함하지 않습니다.

### 전체 일괄 전사 후보 누락 수정 (2026-09-07)

- `scope=all`은 DB 작업과 체크포인트 폴더의 회의 ID를 합쳐 중복 제거합니다.
  전사를 시작한 적 없어 체크포인트가 없는 `recorded` 작업도 후보에 포함됩니다.
  기존 체크포인트 기반 요약 후보 탐색과 상태·경로·오디오 admission은 유지합니다.
- 미리보기와 실행은 같은 후보 계산 경로를 사용합니다. 실제 SQLite에 등록한
  체크포인트 없는 녹음의 preview 포함·무변경, 실행 시 큐잉, 중복 제거,
  처리 중 제외, 오디오 소실 시 409 차단을 격리 테스트로 확인합니다.
- 실제 사용자 데이터의 대상 건수 재측정이나 일괄 전사는 실행하지 않았습니다.
  OpenAI HTTP 400 원인 조사는 이 수정에 포함하지 않습니다.

### 로컬 실행 및 화면 검증 (2026-09-07)

- 현재 검색·날짜 일관성 개선은 커밋되어 릴리스 검증 대상으로 정리되었습니다.
  아래의 '로컬 변경' 기록은 구현 당시 상태이며 현재 게시 상태를 뜻하지 않습니다.
- 기존 서비스와 충돌하지 않는 별도 loopback 포트 및 격리 저장소로 실제
  `main.py --no-menubar`를 실행했습니다. HTTP health, 초기 파일 감사,
  FolderWatcher, JobProcessor 시작을 확인했습니다. macOS `/tmp` symlink 대신
  canonical `/private/tmp` 경로를 사용해 no-follow 저장소 계약을 지켰습니다.
- Terra 독립 브라우저 검증에서 홈·검색·채팅·위키 빈 상태, 준비·일반 설정,
  가상 회의의 전사·요약 표시를 확인했습니다. 840×900 준비·뷰어 화면에서
  가로 잘림을 관찰하지 못했고 준비·설정 추가 검사에서 콘솔·페이지 오류는 0건입니다.
- 준비·설정의 초기 로딩 문구는 응답 후 정상 해소됐습니다. 가상 회의는 UI
  검증용 데이터이며 실제 음성 인식 결과가 아닙니다.
- 검증 장비의 HF CLI 캐시 권한과 Aggregate Device 구성은 미완료입니다.
  실제 녹음→전사→화자분리→요약, 실제 데이터 검색, 네이티브 창 전체 E2E는
  미검증입니다. 사용자 설정·자격 증명·실제 회의 데이터는 변경하지 않았습니다.
- 다음 UI 개선 후보는 상세 작업 버튼 축소, 준비 화면 기술 정보 접기,
  RAM/CPU 표시 비중 축소입니다. 이번 릴리스 구현 범위에는 포함하지 않습니다.
- 릴리스 재검증: 기본 테스트 3911 통과/1 skip, 하네스 152 통과/1 skip,
  전체 lint/format 및 Python 3.12 target production mypy 통과입니다.
- 명시 브라우저 E2E 2개 파일은 17개 테스트 모두 통과했습니다. 최초 실행은
  편집 fixture가 desktop 프로필의 자동 재색인·모델 로드와 결합되어 후속 초기화
  7건이 timeout됐습니다. 편집 서버를 기존 `api-test` 프로필로 격리해 해결했으며,
  이 테스트는 실제 브라우저/API/저장 동작을 검증하고 모델 추론은 검증하지 않습니다.
- 소스 포함 앱의 실제 서버 기동에서 Zoom CoreAudio helper 누락을 발견해
  `scripts/zoom_audio_activity.swift`를 번들 파일 허용 목록에 추가했습니다.
  설치·벤치마크 스크립트 전체를 포함하지 않으며, helper 포함 및 불필요 파일
  제외를 패키지 회귀 테스트로 확인합니다.

### 커밋·배포 준비 검증 (2026-09-07)

- 검색·날짜·전사 수정 일관성 변경을 커밋할 범위로 정리하고 전체 린트 및 포맷을
  재검증했습니다. 감사 재현 스크립트 2개도 프로젝트 포맷에 맞췄습니다.
- `python -m scripts.build_unsigned_release --output-dir dist/review-20260907
  --python .venv/bin/python --bundle-source --json`으로 소스 포함 Recap.app,
  Recap.dmg 및 release manifest를 생성했습니다. 결과는 `local_ready=true`,
  `distribution_ready=false`입니다. 로컬 패키지 생성은 공개 배포 완료를 뜻하지 않습니다.
- 서명·공증된 사용자용 배포와 실제 음성/모델/네이티브 전체 E2E는 아직 미완료입니다.
- 커밋 전 재검증: 기본 테스트 3911 통과/1 skip, 하네스 152 통과/1 skip,
  전체 ruff check/format, Python 3.12 target의 전체 production mypy,
  검색·뷰어·설정 JS 문법 검사 통과.

### 검색·날짜 일관성 후속 보강 (2026-09-05, 로컬 변경)

- `search_receipt`의 running 상태와 알 수 없는 상태를 성공으로 취급하지 않도록
  수정했습니다. 자동 재색인 lease 획득 후 running을 게시하고 시작 이벤트를 보내며,
  서버 재시작 시 running도 복구 대상으로 등록합니다. 화면에서 대기/작업 중을 구분합니다.
- 최초 인덱싱과 재색인이 `preserve_meeting_date`를 공유합니다. 날짜와 출처를
  회의별 `meeting_date.json`에 no-follow/no-replace로 한 번 기록하며, 이후 파일
  mtime 변경·원본 삭제·상충하는 source 날짜가 있어도 보존된 날짜를 유지합니다.
  기존 회의의 잘못된 날짜를 자동 교정하는 migration은 포함하지 않습니다.
- 날짜가 미확정이면 빈 값을 유지합니다. Wiki는 날짜가 필요한 기능이므로
  명시적 보류 이유를 기록하고 전사·검색 결과를 보존합니다.
- 검색 엔진의 실제 호출은 저장소 오류를 삼키지 않는 strict 경로를 사용합니다.
  독립 검색의 정상 결과는 유지하고 `source_errors.vector/fts`를 API에 반환합니다.
  화면은 일부 실패 또는 검색 미완료를 표시하고, 일반/스트리밍 채팅은 검색 일부
  실패 시 LLM 생성을 중단합니다. 임베딩 모델 자체의 로드 실패는 기존 오류 경로입니다.
- 추가 검증: 날짜 보존/실행 중 상태/재시작 등록/실제 FTS·벡터 장애/부분 검색/일반 및
  스트리밍 채팅 차단 회귀. Playwright fixture에서 작업 중 표시 및 두 검색 실패
  화면을 검증했으며 pageerror 0입니다. 실제 사용자 음성·모델 추론 전체 E2E는 미실행입니다.
- 최종 검증: `.venv/bin/python -m pytest tests/ -q --tb=short` **3911 passed,
  1 skipped, 234 deselected**. 하네스 **152 passed, 1 skipped**. 변경 Python 7개
  파일 mypy(`--python-version 3.12 --follow-imports=silent`), 관련 ruff 및
  검색/뷰어/설정 JS syntax 검사 통과. 경고 2건은 기존 Starlette 테스트 런타임 deprecation입니다.

### 현재 시스템 개선 1차 적용 (2026-09-05, 로컬 변경)

- 시작 소스는 `e3e4b61f2d5730e257fa46b79a06bcf1e09d575e`이며, 아래 변경은 아직
  커밋·게시되지 않았습니다. 과거 정리 wave의 커밋 정보와 구분합니다.
- 전사 PUT/replace의 원자적 JSON 저장에 `search_revision`을 포함합니다. 검색
  반영 영수증은 Chroma·FTS와 embed 체크포인트 저장이 모두 성공한 뒤 게시합니다.
  기존 재전사·삭제용 `reindex_required.json` 계약은 그대로 유지합니다.
- 회의별 FIFO lease로 자동 재색인과 연속 편집을 직렬화하고, 시작 감사 후 미반영
  편집본을 재등록합니다. 반영 전/실패한 청크와 질의 도중 교체된 옛 청크를 제외합니다.
  채팅 이력을 교체하고 생성 도중 편집된 답변을 취소해 이후 질문에 옛 답변이 섞이지
  않도록 합니다. 이미 전송된 스트리밍 토큰을 소급해 지우는 UI는 포함하지 않습니다.
- WebSocket accept 전 같은 loopback Origin/Host/포트를 확인합니다. 범위가 있는
  Wiki 질문은 RAG에 위임하고, 범위 지정 RAG는 이전 대화 이력을 사용하지 않습니다.
- CSV 화자는 실제 Chroma `$in`과 FTS 구분자 일치로 검색합니다. 기존 저장 형식을
  유지하며, 화자 질의 시 metadata 선별 비용이 생깁니다.
- 재색인 날짜는 보존 값 → 날짜 파일명 → 검증된 원본 시각 순으로 결정하며,
  확인 불가능한 날짜는 빈 값으로 보존합니다. 최초 파이프라인 전체의 날짜 출처
  통일과 기존 데이터 충돌 검사는 후속 작업입니다.
- 검색 상태는 Chroma/FTS/체크포인트 청크 ID 집합과 편집 버전을 비교합니다.
  설정에서 복구 사유, 뷰어에서 검색 반영 및 요약 확인 필요, 리소스 바에서
  이전 native 작업 정리 대기를 표시합니다.
- 검증: Python 3.12 지원 환경에서 기본 테스트 **3,905 통과, 1 skip, 234 deselect**;
  harness **152 통과, 1 skip**; 변경 Python 9개 파일 mypy(`--python-version 3.12`),
  관련 ruff, JS 3개 syntax 검사 통과. 기본 mypy의 3.11 target은 설치된 NumPy
  2.5 stub의 3.12 문법 때문에 파싱이 막혀, 실제 실행 환경과 같은 target으로 검증했습니다.
- Playwright Chromium 148에서 실제 화면 모듈 + fixture API의 재색인 버튼·상태
  변경을 검증했습니다. 콘솔 pageerror 0, 데스크톱 1200×850 및 모바일 390×844의
  문구 표시 확인. 실제 사용자 음성·모델 추론·네이티브 창 전체 E2E는 실행하지 않았습니다.
- 남은 범위: 계획서의 모든 완료 조건을 닫은 상태는 아닙니다. pending의 대기/실행
  세분화, Wiki·요약 원문 버전 연결, 최초 날짜 출처 통일, 검색 소스별 장애 응답,
  상세 무결성 검사 분리, 백업·복원·패키징·온보딩은 후속 단계로 추적합니다.
