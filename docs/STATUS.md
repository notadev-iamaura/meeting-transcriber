...
- 기준일: 2026-05-07
- 기준 브랜치: `main`
- 기준 커밋: `2fec123f1d630eea1c3b24460e2cd126d2bd49df`
- 최근 정리 wave: #41 → #38 → #39 → #40 → #42 → #43 → #44 → #45 → #46 → #47 → #48 모두 main 반영
...
- `api/routes.py`는 app-state dependency helper, meetings batch, STT models,
  wiki/reindex, settings/user-settings, search/chat, meeting detail router
  분리를 완료했습니다.
...
Phase G의 목적은 시스템 상태/대시보드/업로드/녹음 API를 `api/routes.py`에서
분리하여 백엔드 라우터 경계를 더 작고 검증 가능한 단위로 만드는 것입니다.

이번 Phase G에서 다루는 API 범위:

- `/api/status`
- `/api/system/resources`, `/api/dashboard/stats`, `/api/system/open-audio-folder`
- `/api/uploads`
- `/api/recording/status`, `/api/recording/start`, `/api/recording/stop`, `/api/recording/devices`

`api.routes`의 기존 response model/helper import 경로와 `api.routes.sys` 같은
테스트 monkeypatch 경로는 re-export/compat shim 으로 유지합니다.
...
1. `api/routes.py` domain router 분리를 계속합니다. STT models는 #43에서
   ...
   meeting detail은 #48에서 분리했습니다. Phase G 후보는
   system/recording/upload routes이며, 이후 후보는 A/B test routes입니다.
