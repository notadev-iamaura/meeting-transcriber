...
| 설명 (13px/400) | `#86868B` | `#F5F5F7` | **3.33:1** | ≥4.5:1 | ❌ AA 미달 — 후술 |
...
| 설명 (13px/400) | `#98989D` | `#2C2C2E` | **4.85:1** | ≥4.5:1 | ✅ AA 통과 |
...
**다크 모드는 본문 색대비 (5.93:1, 4.85:1) 가 AA 4.5:1 을 충족** 하므로 design.md 토큰을 그대로 따를 때 문제 없음.

**라이트 모드의 3 가지 잠재 위반 — T-102 에서 보강 완료**:

본 §6.1 의 측정값은 *T-101 머지 시점* 의 design.md 토큰 (`--text-secondary: #86868B` / `--text-muted: #AEAEB2` / `--accent: #007AFF`) 을 기준으로 한 것이며, **T-102 (`dark-mode-tones`) 에서 라이트 토큰을 보강해 모든 항목이 AA/UI 1.4.11 통과로 전환됨**. T-102 머지 후 본 §6.1 표는 다음 새 측정값으로 효력 대체:

| 요소 | 새 토큰 | 배경 | 새 측정 | 기준 | 결과 |
|------|--------|------|---------|------|------|
| 설명 (text-secondary) | `#6E6E73` | `#FFFFFF` | **5.07:1** | ≥4.5:1 | ✅ AA 통과 |
| 아이콘 (text-muted) | `#8E8E93` | `#FFFFFF` | **3.26:1** | ≥3:1 (1.4.11) | ✅ 통과 |
| CTA 라벨 (accent-text) | `#0066CC` (신규 토큰) | `#FFFFFF` | **5.57:1** | ≥4.5:1 | ✅ AA 통과 |
| CTA 보더 (accent) | `#007AFF` (보존) | `#FFFFFF` | 4.02:1 | ≥3:1 (UI) | ✅ 통과 |

세부 정정 사항:

1. **`text-secondary` (#86868B → #6E6E73)** — 보강 완료. 5.07:1 (AA 통과). SPA 전역 secondary 텍스트 100+ 개소 자동 동기화.

2. **`text-muted` (#AEAEB2 → #8E8E93)** — 보강 완료. 3.26:1 (UI 1.4.11 통과). 아이콘에 대한 비-텍스트 콘텐츠 기준 충족.

3. **`accent` 본문 텍스트 4.5:1 미달** — `--accent` (`#007AFF`) 자체는 보더·시스템 강조용으로 보존하고, 신규 토큰 `--accent-text` (`#0066CC`, 5.57:1) 를 본문 CTA 라벨 용도로 도입.

→ T-101 의 a11y 테스트 (`tests/ui/a11y/test_empty_state.py::test_empty_state_no_axe_violations`) 의 `color-contrast` 룰도 T-102 와 함께 재활성화 완료.

### 6.4 본 티켓의 결론

본 빈 상태 컴포넌트 자체는 design.md 토큰을 100% 따른다 (재정의·새 토큰 도입 없음). T-101 머지 시점에서는 라이트 모드 본문 텍스트가 design.md 토큰의 알려진 한계로 AA 미달이었고 후속 티켓 (T-102 `dark-mode-tones`) 으로 회부. **T-102 가 라이트 토큰을 보강하면서 위 3 항목 모두 AA/UI 1.4.11 통과** 로 전환되었으며, 다크 모드는 원래부터 AA 통과였으므로 라이트/다크 모두 자동으로 합격 상태가 되었다.

> 📌 T-102 머지 완료 후 본 §6.3 의 우려 사항은 효력 상실 (라이트 토큰 보강으로 해소).
